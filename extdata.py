...
class ExtData():
    ...
    def calc_ext_elvebv(self, reddened_star, model_fluxes_bands, model_fluxes_spectra, ebv):
        ...
        const_norm = 2.5*math.log10((_model_ave/_data_ave)*reddened_star.data['BANDS'].flat_bands_fluxes[vk]/model_fluxes_bands[vk])
        ...
        self.ext_curve['BANDS'] = (_model_ave/_data_ave)*(reddened_star.data['BANDS'].flat_bands_fluxes/model_fluxes_bands)
        self.ext_curve['BANDS'] = -2.5*np.log10(self.ext_curve['BANDS']) + const_norm
        ...
        self.ext_curve['STIS'] = (_model_ave/_data_ave)*(reddened_star.data['STIS'].flux[sindxs_good]/model_fluxes_spectra)
        self.ext_curve['STIS'] = -2.5*np.log10(self.ext_curve['STIS']) + const_norm
    ...
    def read_ext_data(self, ext_filename):
        ...
        if pheader.get('EBV'):
            self.ebv = (float(pheader.get('EBV')), float(pheader.get('EBV_UNC')))
        if pheader.get('RV'):
            self.rv = (float(pheader.get('RV')), float(pheader.get('RV_UNC')))
        if pheader.get('LOGHI'):
            self.loghi = (float(pheader.get('LOGHI')), float(pheader.get('LOGHI_U')))
        if pheader.get('LOGHIMW'):
            self.loghimw = (float(pheader.get('LOGHIMW')), float(pheader.get('LHIMW_U')))
        if pheader.get('NHIAV'):
            self.nhiav = (float(pheader.get('NHIAV')), float(pheader.get('NHIAV_U')))

        if pheader.get('FMC2'):
            self.fm90 = {}
            self.fm90['C2'] = (float(pheader.get('FMC2')), float(pheader.get('FMC2U')))
            self.fm90['C3'] = (float(pheader.get('FMC3')), float(pheader.get('FMC3U')))
            self.fm90['C4'] = (float(pheader.get('FMC4')), float(pheader.get('FMC4U')))
            self.fm90['x0'] = (float(pheader.get('FMx0')), float(pheader.get('FMx0U')))
            self.fm90['gamma'] = (float(pheader.get('FMgam')), float(pheader.get('FMgamU')))
            # for completeness, populate C1 using from the FM07 relationship
            self.fm90['C1'] = (2.09 - 2.84*self.fm90['C2'][0], 2.84*self.fm90['C2'][1])
    ...
    def ext_uncs(self, mcmc_samples):
        ...
        for i in range(n_waves):
            for j in range(n_waves):
                for k in range(nsamp):
                    self.covar_matrix[i,j] += (ext_curves[i,k] - ave_ext[i])*(ext_curves[j,k] - ave_ext[j])
        ...
        for i in range(n_waves):
            for j in range(n_waves):
                self.corr_matrix[i,j] /= (self.ext_curve_uncs_from_covar[i]*self.ext_curve_uncs_from_covar[j])

        # now break the straight uncertainties apart to make it easy to save
        self.ext_curve_uncs = {}
        self.ext_curve_uncs['BANDS'] = self.ext_curve_uncs_from_covar[0:len(self.ext_x['BANDS'])]
        self.ext_curve_uncs['STIS'] = self.ext_curve_uncs_from_covar[len(self.ext_x['BANDS']):]
    ...
    def save_ext_data(self, ext_filename, fit_param, fit_param_uncs):
        ...
        hname = ['EXTTYPE','IUEDATA','FUSEDATA','OPTDATA','NIRDATA','IRSDATA',
                 ...
                 'AV','AV_unc','RV','RV_unc',
                 'FMC2','FMC2U','FMC3','FMC3U','FMC4','FMC4U','FMx0','FMx0U','FMgam','FMgamU',
                 'LOGHI','LOGHI_U','LOGHIMW','LHIMW_U',
                 'EBV','EBV_UNC',
                 'NHIAV','NHIAV_U','NHIEBV','NHIEBV_U']
        hval = np.concatenate([[self.ext_type, 1, 0, 0, 0, 0, self.reddened_file, 'TLUSTY MODELS'],fit_param_packed])
        ...
        col1 = fits.Column(name='WAVELENGTH', format='E', array=self.ext_waves['BANDS'])
        col2 = fits.Column(name='X', format='E', array=self.ext_x['BANDS'])
        col3 = fits.Column(name='EXT', format='E', array=self.ext_curve['BANDS'])
        col4 = fits.Column(name='UNC', format='E', array=self.ext_curve_uncs['BANDS'])
        ...
        col1 = fits.Column(name='WAVELENGTH', format='E', array=self.ext_waves['STIS'])
        col2 = fits.Column(name='X', format='E', array=self.ext_x['STIS'])
        col3 = fits.Column(name='EXT', format='E', array=self.ext_curve['STIS'])
        col4 = fits.Column(name='UNC', format='E', array=self.ext_curve_uncs['STIS'])
        cols = fits.ColDefs([col1, col2, col3, col4])
        tbhdu = fits.BinTableHDU.from_columns(cols)
        ...
        hdu.writeto(string.replace(ext_filename,'.fits','_covar.fits'), clobber=True)
